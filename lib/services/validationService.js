 ...
 * CRITICAL: Remove prompt injection patterns
 */
function sanitizeContext(context) {
  ...
  return String(context)
    .replace(/[\n\r]/g, " ") // Remove newlines
    .replace(/IGNORE|RETURN|SYSTEM|PROMPT|INSTRUCTION|OVERRIDE|EXECUTE|RUN|EVAL/gi, "") // Remove keywords
...
export function validateContext(context) {
  ...
  const wasSanitized = sanitized !== context.slice(0, MAX_CONTEXT_LENGTH);
